...
<div id="wrapper">
	...
	<form action="<?php echo URL_ROOT . '/add_products_to_inventory.php' ?>" method="post">
		<!-- added: URL ROOT doesn't need quotation marks -->
    <?php
		//create a loop that writes a form to allow up to 8 produce items to be added
		for ($x = 0; $x <= 7; $x++)  {?>
			<!-- added: fixed where the HTML and PHP are writing -->
			<!-- added: put brace before the PHP close so it wouldn't write as HTML -->
    <fieldset>
...
<?php } ?> <!-- added: a brace -->
<!-- added: correct PHP tags so the closing brace would write-->
